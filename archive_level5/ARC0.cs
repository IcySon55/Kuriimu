    ...
    public sealed class ARC0
    {
        ...
        public void Save(Stream output)
        {
            using (BinaryWriterX bw = new BinaryWriterX(output))
            {
                ...
                foreach (var folderCount in folderCounts)
                {
                    var nameSorted = new List<ARC0NameEntry>();
                    for (int i = 0; i < folderCount; i++) nameSorted.Add(new ARC0NameEntry { name = Files[pos + i].FileName, crc32 = Files[pos + i].crc32, size = (uint)Files[pos + i].FileSize });
                    nameSorted = nameSorted.OrderBy(x => x.name, StringComparer.OrdinalIgnoreCase).ToList();

                    var entriesTmp = new List<ARC0Entry>();
                    int nameOffset = 0;
                    for (int i = 0; i < folderCount; i++)
                    {
                        entriesTmp.Add(new ARC0Entry { crc32 = Files[pos + i].crc32 });
                    }
                    for (int i = 0; i < folderCount; i++)
                    {
