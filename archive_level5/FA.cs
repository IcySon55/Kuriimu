...
namespace archive_fa
{
    public sealed class FA
    {
        ...
        private FAHeader header;
        private byte[] unk1;
        private byte[] unk2;

        private List<FAEntry> entries;
        ...
        public FA(Stream input)
        {
            ...
            using (BinaryReaderX br = new BinaryReaderX(input, true))
            {
                ...
                //Entries
                entries = br.ReadMultiple<FAEntry>(header.fileCount);
                ...
                {
                    var tmpFiles = new List<FANameEntry>();
                    for (int i = 0; i < folderCount; i++)
                        tmpFiles.Add(new FANameEntry
                        {
        ...
        public void Save(Stream output)
        {
            using (BinaryWriterX bw = new BinaryWriterX(output))
            {
                ...
                foreach (var folderCount in folderCounts)
                {
                    var nameSorted = new List<FANameEntry>();
                    for (int i = 0; i < folderCount; i++) nameSorted.Add(new FANameEntry { name = Files[pos + i].FileName, crc32 = Files[pos + i].crc32, size = (uint)Files[pos + i].FileSize });
                    nameSorted = nameSorted.OrderBy(x => x.name, StringComparer.OrdinalIgnoreCase).ToList();

                    var entriesTmp = new List<FAEntry>();
                    uint nameOffset = 0;
                    for (int i = 0; i < folderCount; i++)
                    {
                        entriesTmp.Add(new FAEntry { crc32 = Files[pos + i].crc32 });
                    }
                    for (int i = 0; i < folderCount; i++)
                    {
