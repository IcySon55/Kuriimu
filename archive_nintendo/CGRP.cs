...
namespace archive_cgrp
{
    public sealed class CGRP
    {
        ...
        private CGRPHeader header;
        private List<CGRPInfo1> info1;
        private List<CGRPInfoEntry> infos;
        private byte[] infx;
        ...
        public CGRP(Stream input)
        {
            ...
            {
                //Header
                header = br.ReadStruct<CGRPHeader>();

                //infoSec
                br.ReadStruct<CGRPPartitionHeader>();
                int fileCount = br.ReadInt32();
                info1 = new List<CGRPInfo1>();
                infos = new List<CGRPInfoEntry>();
                for (int i = 0; i < fileCount; i++)
                    info1.Add(br.ReadStruct<CGRPInfo1>());
                for (int i = 0; i < fileCount; i++)
                    infos.Add(br.ReadStruct<CGRPInfoEntry>());
