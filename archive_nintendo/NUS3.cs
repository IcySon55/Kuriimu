...
namespace archive_nus3bank
{
    public sealed class NUS3
    {
        ...
        public NUS3Header header;
        public NUS3BankToc banktocHeader;
        public List<NUS3BankTocEntry> banktocEntries;
        public NUS3PROP prop;
        public NUS3BINF binf;
        public byte[] grp;
        public byte[] dton;
        public NUS3TONE tone;
        public byte[] junk;
        ...
        public NUS3(string filename, bool isZlibCompressed = false)
        {
            ...
            {
                //Header
                header = br.ReadStruct<NUS3Header>();

                //Banktoc
                banktocHeader = br.ReadStruct<NUS3BankToc>();

                int offset = 0x18 + banktocHeader.entryCount * 0x8;
                banktocEntries = new List<NUS3BankTocEntry>();
                for (int i = 0; i < banktocHeader.entryCount; i++)
                {
                    banktocEntries.Add(new NUS3BankTocEntry(br.BaseStream));
                    banktocEntries[i].offset = offset;
                    offset += banktocEntries[i].secSize + 8;
                }

                //PROP
                if (br.ReadStruct<NUS3Header>().magic != "PROP") throw new Exception();
                prop = new NUS3PROP(br.BaseStream);

                //BINF
                br.BaseStream.Position = banktocEntries[1].offset;
                if (br.ReadStruct<NUS3Header>().magic != "BINF") throw new Exception();
                binf = new NUS3BINF(br.BaseStream);

                //GRP - not yet mapped
                br.BaseStream.Position = banktocEntries[2].offset;
                if (br.ReadStruct<NUS3Header>().magic != "GRP ") throw new Exception();
                grp = br.ReadBytes(banktocEntries[2].secSize);

                //DTON - not yet mapped
                br.BaseStream.Position = banktocEntries[3].offset;
                if (br.ReadStruct<NUS3Header>().magic != "DTON") throw new Exception();
                dton = br.ReadBytes(banktocEntries[3].secSize);

                //TONE
                br.BaseStream.Position = banktocEntries[4].offset;
                if (br.ReadStruct<NUS3Header>().magic != "TONE") throw new Exception();
                tone = new NUS3TONE(br.BaseStream, banktocEntries[4].offset, banktocEntries[6].offset);

                //JUNK - not yet mapped
                br.BaseStream.Position = banktocEntries[5].offset;
                if (br.ReadStruct<NUS3Header>().magic != "JUNK") throw new Exception();
                junk = br.ReadBytes(banktocEntries[5].secSize);

                //PACK and finishing
                br.BaseStream.Position = banktocEntries[6].offset;
                if (br.ReadStruct<NUS3Header>().magic != "PACK") throw new Exception();
