...
namespace text_bmg
{
    public class BmgAdapter : ITextAdapter
    {
        private FileInfo _fileInfo = null;
        private BMG _bmg = null;
        private BMG _bmgBackup = null;
        ...
        public bool Identify(string filename)
        {
            ...
            {
                return br.ReadString(8) == "MESGbmg1";
            }
        }
        ...
        public LoadResult Load(string filename, bool autoBackup = false)
        {
            ...
            if (_fileInfo.Exists)
            {
                _bmg = new BMG(_fileInfo.FullName);

                string backupFilePath = _fileInfo.FullName + ".bak";
                if (File.Exists(backupFilePath))
                {
                    _bmgBackup = new BMG(backupFilePath);
                }
        ...
        public IEnumerable<TextEntry> Entries
        {
            get
            {
                if (_entries == null)
                {
                    ...
                    foreach (Label label in _bmg.Labels)
                    {
                        if (_bmgBackup == null)
                        {
                        ...
                        {
                            Entry entry = new Entry(label, _bmgBackup.Labels.FirstOrDefault(o => o.TextID == label.TextID));
                            _entries.Add(entry);
                        }
                    }
                }
