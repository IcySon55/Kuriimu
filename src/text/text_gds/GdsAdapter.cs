...
namespace text_gds
{
    public sealed class TTBinAdapter : ITextAdapter
    {
        ...
        private GDS _gds = null;
        private GDS _gdsBackup = null;
        private List<Entry> _entries = null;

        #region Properties

        // Information
        public string Name => "GDS";
        ...
        public bool Identify(string filename)
        {
            using (var br = new BinaryReaderX(File.OpenRead(filename)))
            {
                ...
                return size == br.BaseStream.Length - 4;
            }
        }
        ...
        public LoadResult Load(string filename, bool autoBackup = false)
        {
            ...
            if (_fileInfo.Exists)
            {
                _gds = new GDS(_fileInfo.FullName);

                string backupFilePath = _fileInfo.FullName + ".bak";
                if (File.Exists(backupFilePath))
                {
                    _gdsBackup = new GDS(backupFilePath);
                }
        ...
        public IEnumerable<TextEntry> Entries
        {
            get
            {
                if (_entries == null)
                {
                    ...
                    foreach (Label label in _gds.Labels)
                    {
                        if (_gdsBackup == null)
                        {
                        ...
                        {
                            Entry entry = new Entry(label, _gdsBackup.Labels.FirstOrDefault(o => o.TextID == label.TextID));
                            _entries.Add(entry);
                        }
                    }
                }
