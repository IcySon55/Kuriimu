...
namespace text_cnvrs
{
    public sealed class TTBinAdapter : ITextAdapter
    {
        ...
        private CNVRS _cnvrs = null;
        private CNVRS _cnvrsBackup = null;
        private List<Entry> _entries = null;

        #region Properties

        // Information
        public string Name => "CNVRS";

        public string Description => "CoNVeRSe";

        public string Extension => "*.cnvrs-text";

        public string About => "This is the CNVRS text adapter for Kuriimu.";
        ...
        public LoadResult Load(string filename, bool autoBackup = false)
        {
            ...
            if (_fileInfo.Exists)
            {
                _cnvrs = new CNVRS(_fileInfo.FullName);

                string backupFilePath = _fileInfo.FullName + ".bak";
                if (File.Exists(backupFilePath))
                {
                    _cnvrsBackup = new CNVRS(backupFilePath);
                }
        ...
        public SaveResult Save(string filename = "")
        {
            ...
            try
            {
                _cnvrs.Save(_fileInfo.FullName);
            }
            catch (Exception)
            {
        ...
        public IEnumerable<TextEntry> Entries
        {
            get
            {
                if (_entries == null)
                {
                    ...
                    foreach (Label label in _cnvrs.Labels)
                    {
                        if (_cnvrsBackup == null)
                        {
                        ...
                        {
                            Entry entry = new Entry(label, _cnvrsBackup.Labels.FirstOrDefault(o => o.TextID == label.TextID));
                            _entries.Add(entry);
                        }
                    }
                }
