...
namespace archive_gar
{
    public sealed class GAR
    {
        ...
        public GARHeader header;
        public List<GARChunkEntry> entries = new List<GARChunkEntry>();
        public List<List<int>> chunkFileIDs = new List<List<int>>();
        public List<string> chunkNames = new List<string>();
        public List<GARChunkInfo> chunkInfos = new List<GARChunkInfo>();
        ...
        public GAR(Stream input)
        {
            ...
            {
                //Header
                header = br.ReadStruct<GARHeader>();

                //FileEntries
                entries = br.ReadMultiple<GARChunkEntry>(header.fileChunks);
                ...
                foreach (var chunk in entries)
                {
                    for (int i = 0; i < chunk.fileCount; i++)
                        chunkInfos.Add(new GARChunkInfo(br.BaseStream));
                }
