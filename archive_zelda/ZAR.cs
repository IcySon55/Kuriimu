...
namespace archive_zar
{
    public sealed class ZAR
    {
        ...
        public ZARHeader header;
        public List<ZARChunkEntry> entries = new List<ZARChunkEntry>();
        public List<List<int>> chunkFileIDs = new List<List<int>>();
        public List<string> chunkNames = new List<string>();
        public List<ZARChunkInfo> chunkInfos = new List<ZARChunkInfo>();
        ...
        public ZAR(Stream input)
        {
            ...
            {
                //Header
                header = br.ReadStruct<ZARHeader>();

                //FileEntries
                entries = br.ReadMultiple<ZARChunkEntry>(header.fileChunks);
                ...
                foreach (var chunk in entries)
                {
                    for (int i = 0; i < chunk.fileCount; i++)
                        chunkInfos.Add(new ZARChunkInfo(br.BaseStream));
                }
