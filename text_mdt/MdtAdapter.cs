...
namespace file_mdt
{
    public sealed class TdtAdapter : IFileAdapter
    {
        ...
        private MDT _mdtBackup = null;
        private List<Entry> _entries = null;

        #region Properties

        // Information
        public string Name => "MDT";

        public string Description => "Theatrythm Message Files";
        ...
        public bool Identify(string filename)
        {
            ...
            {
                if (br.ReadString(4) == "MSGD") return true;
        ...
        public LoadResult Load(string filename, bool autoBackup = false)
        {
            ...
            if (_fileInfo.Exists)
            {
                _mdt = new MDT(_fileInfo.FullName);

                string backupFilePath = _fileInfo.FullName + ".bak";
                if (File.Exists(backupFilePath))
                {
                    _mdtBackup = new MDT(backupFilePath);
                }
